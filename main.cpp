...
const char *TCPING_VERSION = "0.40";
const char *TCPING_DATE = "Oct 4 2020";
...
{
	cout << "--------------------------------------------------------------" << endl;
	cout << "tcping version " << TCPING_VERSION << endl;
	cout << "tcping - ping over a tcp connection" << endl;
	cout << "License under the GPL." << endl;
	cout << "--------------------------------------------------------------" << endl;
	cout << endl;
	cout << "Usage: " << argv[0] << " [-flags] server-address [server-port]" << endl
		 << endl;
	cout << "       " << argv[0] << " [-t] [-d] [-i interval] [-n times] [-w ms] [-b n] [-r times] [-s] [-v] [-j] [-js size] [-4] [-6] [-c] [-g count] [-S source_address] [--file] [-o filename] [-h] [-u] [--post] [--head] [--proxy-port port] [--proxy-server server] [--proxy-credentials username:password] [-f] server-address "
		 << "[server-port]" << endl
		 << endl;
	...
	cout << " -s     : automatically exit on a successful ping" << endl;
	...
	cout << " -o     : print console to a file, filename after '-o'" << endl;
	cout << " --append : Append to the -o filename rather than overwriting it" << endl;
	...
	cout << endl
		 << "HTTP Options:" << endl;
	cout << " -h     : HTTP mode (use address without http://)" << endl;
	...
	cout << " --header : include a header with original args and date.  Implied if using -o." << endl;
	...
	cout << endl
		 << "\tIf you don't pass server-port, it defaults to " << kDefaultServerPort << "." << endl;
	cout << "\tAlso you can use 443 as default HTTPS server port." << endl;
}
...
int main(int argc, char *argv[])
{
	...
	for (int x = 0; x < argc; x++)
	{
		...
		if (!strcmp(argv[x], "-t"))
		{
			...
			offset = x;
			cout << endl
				 << "** Pinging continuously. Press Ctrl-C to stop **" << endl;
		}
		...
		if (!strcmp(argv[x], "-c"))
		{
			...
			offset = x;
			cout << endl
				 << "** Only displaying output for state changes. **" << endl;
		}
		...
		if (!strcmp(argv[x], "-b"))
		{
			...
			case 1:
				cout << endl
					 << "** Beeping on \"down\" - (two beeps) **" << endl;
				break;
			case 2:
				cout << endl
					 << "** Beeping on \"up\"  - (one beep) **" << endl;
				break;
			case 3:
				cout << endl
					 << "** Beeping on \"change\" - (one beep up, two beeps down) **" << endl;
				break;
			case 4:
				cout << endl
					 << "** Beeping constantly - (one beep up, two beeps down) **" << endl;
				break;
			}
		}
	...
	{
		cout << nNumArgsIgnored << " extra argument" << (nNumArgsIgnored == 1 ? "" : "s") << " ignored." << endl;
	}
